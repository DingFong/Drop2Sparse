...
class ConvNet_dropout(nn.Module):
    def __init__(self,
                 ...
                 dropout_rate=0.2,
                 im_size=(32, 32)):
        # print(f"Define Convnet_dropout (depth {net_depth}, width {net_width}, norm {net_norm})")
...
class ConvNet_mask(nn.Module):
    def __init__(self,
        ...
        num_feat = shape_feat[0] * shape_feat[1] * shape_feat[2]
        self.classifier = nn.Linear(num_feat, num_classes)
        self.mask = torch.autograd.Variable(torch.bernoulli(torch.ones((1, num_feat))*(1-dropout_rate)))/dropout_rate
