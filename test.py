    ...
    if args.dataset == 'imagenet':
        ...
        if 'idc' in args.slct_type:
            if args.slct_type == 'idc':
                if args.niter:
                    data, target = torch.load(os.path.join(f'{args.save_dir}', f'data{args.niter}.pt'))
                else:
                    data, target = torch.load(os.path.join(f'{args.save_dir}', f'data.pt'))
                ...
                for idx in range(args.nclass // args.nclass_sub):
                    ...
                    if args.niter:
                        data, target = torch.load(os.path.join(f'{path}', f'data{args.niter}.pt'))
                    else:
                        data, target = torch.load(os.path.join(f'{path}', f'data.pt'))
...
def test_data(args,
    ...
    for model_fn in model_fn_ls:
        ...
        for _ in range(repeat):
            ...
            if args.track_training:
                return train(args, model, train_loader, val_loader, logger=print)
    ...
    return np.mean(best_acc_l), np.mean(acc_l), np.std(best_acc_l), np.std(acc_l)
...
if __name__ == '__main__':
    ...
    wandb.init(sync_tensorboard=False,
               ...
               job_type = "CleanRepo",
               config = args,
               name = f"Test_{args.name}_Iters:{args.niter}"
    )
    ...
    for p in path_list:
        ...
        result = test_data(args, train_loader, val_loader, repeat=args.repeat, test_resnet=False)
        if args.track_training:
            for i in range(args.epochs// args.epoch_print_freq):
                print(len(result['loss_tr']))
                wandb.log({
                    'Tracking/loss_tr': result['loss_tr'][i],
                    'Tracking/loss_val': result['loss_val'][i],
                    'Tracking/acc_tr': result['acc_tr'][i],
                    'Tracking/acc_val': result['acc_val'][i]
                })

        if args.dataset[:5] == 'cifar':
            result_resnet = test_data(args, train_loader, val_loader, repeat=args.repeat, model_fn=resnet10_bn)
            if (not args.same_compute) and (args.ipc >= 50 and args.factor > 1):
                args.epochs = 400
            result_densenet = test_data(args, train_loader, val_loader, repeat=args.repeat, model_fn=densenet)

            # save acc top1 acc result[0] of convnet、resnet and densenet in wandb
            wandb.log({
                'Top1_acc_mean/convnet': result[0], 
                'Top1_acc_mean/resnet': result_resnet[0], 
                'Top1_acc_mean/densenet': result_densenet[0],
                'Top1_acc_std/convnet': result[2],
                'Top1_acc_std/resnet': result_resnet[2],
                'Top1_acc_std/densenet': result_densenet[2],
                })
        
        
        elif args.dataset == 'imagenet':
            result_resnet = test_data(args, train_loader, val_loader, repeat=args.repeat, model_fn=resnet18_bn)
            result_efficient = test_data(args, train_loader, val_loader, repeat=args.repeat, model_fn=efficientnet)

            # save acc top1 acc result[0] of convnet、resnet and efficient in wandb
            wandb.log({
                'Top1_acc/resnet10_ap': result[0], 
                'Top1_acc/resnet18': result_resnet[0], 
                'Top1_acc/efficientb0': result_efficient[0],
                'Top1_acc_std/resnet10_ap': result[2],
                'Top1_acc_std/resnet18': result_resnet[2],
                'Top1_acc_std/efficientb0': result_efficient[2],
                })