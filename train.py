...
def define_model(args, nclass, logger=None, size=None):
    ...
    if args.net_type == 'resnet':
        print(f"resnet, dataset:{args.dataset}, depth = {args.depth}, norm_type: {args.norm_type}, nch = {args.nch}")
        ...
        print(f"convnet, dataset:{args.dataset}, depth = {args.depth}, norm_type: {args.norm_type}, nch = {args.nch}")
...
def train(args, model, train_loader, val_loader, plotter=None, logger=None):
    ...
    # print(get_time())
    process = {'loss_tr': [], 'loss_val': [], 'acc_tr': [], 'acc_val': []}
    if args.track_training:
        args.epoch_print_freq = 1
    
    # test random initialized model performance:
    test_train(args,
            train_loader,
            model,
            criterion,
    ...
    for epoch in range(cur_epoch+1, args.epochs + 1):
        ...
        if epoch % args.epoch_print_freq == 0:
            ...
            process['loss_tr'].append(loss_tr)
            process['loss_val'].append(loss_val)
            process['acc_tr'].append(acc1_tr)
            process['acc_val'].append(acc1)
        ...
        if args.save_ckpt:
            ...
            save_checkpoint(args.save_dir, state, None)
        scheduler.step()
...
def test_train(args,
    ...
    if epoch == 0 and (logger is not None):
        logger(
            '(Train) [Epoch {0}/{1}] {2} Top1 {top1.avg:.1f}  Top5 {top5.avg:.1f}  Loss {loss.avg:.3f}'
            .format(epoch, args.epochs, get_time(), top1=top1, top5=top5, loss=losses))
...
def save_checkpoint(save_dir, state, is_best):
    ...
    if is_best:
        ...
        ckpt_path = os.path.join(save_dir, f'checkpoint{epoch}.pth.tar')
        torch.save(state, ckpt_path)
        print("checkpoint saved! ", ckpt_path)
...
if __name__ == '__main__':
    ...
    args.save_dir += f'/_{args.epochs}_{args.seed}'
